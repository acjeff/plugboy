	...
	if (instance_exists(oGlowbie) && instance_exists(oPlayer)) {
		...
		if (!dead) {
			if (discharge) {
			 ...
			 if (beingAttacked && !global.paused) {	
			 ...
			 if (throwEnergyBallTimer > 0 && throwEnergyBall && !global.paused) {
			...
			if (throwEnergyBallTimer <= 0 && !global.paused) {
